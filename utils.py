...
def prepare_data(config, device='cuda'):
    ...
    elif data_config['type'] == 'generic':
        # Any dataset registered in sesil.data.DATASETS (SVHN, MNIST, STL-10,
        # ...). CIFAR keeps the branch above, unchanged.
        from datasets.generic import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)
